...
protected:
    ...
    void load_bloom_filter(std::string bf_path) {
        ...
        if (is) {
            ...
            is.read(reinterpret_cast<char*>(&salt_size), sizeof(unsigned int));
            salt_.resize(salt_size);

            for (int i = 0; i < salt_size; i++) {
                is.read(reinterpret_cast<char*>(&salt_[i]), sizeof(unsigned int));
            }
            

            unsigned int bit_table_size;
            is.read(reinterpret_cast<char*>(&bit_table_size), sizeof(unsigned int));
            bit_table_.resize(bit_table_size);

            for (int i = 0; i < bit_table_size; i++) {
                is.read(reinterpret_cast<char*>(&bit_table_[i]), sizeof(unsigned char));
            }

            is.read(reinterpret_cast<char*>(&salt_count_), sizeof(unsigned int));
            is.read(reinterpret_cast<char*>(&table_size_), sizeof(unsigned long long int));
            is.read(reinterpret_cast<char*>(&projected_element_count_), sizeof(unsigned long long int));
            is.read(reinterpret_cast<char*>(&inserted_element_count_), sizeof(unsigned long long int));
            is.read(reinterpret_cast<char*>(&random_seed_), sizeof(unsigned long long int));
            is.read(reinterpret_cast<char*>(&desired_false_positive_probability_), sizeof(double));
